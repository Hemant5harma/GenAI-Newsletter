...
interface GenerationSettings {
    // Legacy mapping
    images: { mode: 'random' | 'manual'; urls: string[]; count: number }; // Added count
    ...
    // Layout
    layoutStyle?: { mode: 'auto' } | { mode: 'manual'; value: 'digest' | 'magazine' | 'curated' | 'roundup' };
    headerStyle?: { mode: 'auto' } | { mode: 'manual'; value: 'masthead' | 'minimal' | 'personal' | 'date_first' };

    // Design
    colors: { mode: 'random' | 'manual'; primary: string; secondary: string; preset?: string }; // Existing matches requirement
    fonts?: { mode: 'auto' } | { mode: 'manual'; value: 'sans' | 'serif' | 'mono' };
    borderRadius?: { mode: 'auto' } | { mode: 'manual'; value: string };
...
// ═══════════════════════════════════════════════════════════════
// MAIN GENERATION FUNCTION - 5-AGENT PIPELINE
// ═══════════════════════════════════════════════════════════════
export async function generateNewsletterForBrand(brandId: string): Promise<string> {
    ...
    // EXTRACT PARAMETERS (Auto vs Manual)
    const timeRange = settings.timeRange?.mode === 'manual' ? settings.timeRange.value : '48h'; // Auto default to 48h
    const tone = settings.tone?.mode === 'manual' ? settings.tone.value : (brand.tone || 'witty, smart, and conversational');
    const wordCount = settings.wordCount?.mode === 'manual' ? settings.wordCount.value : 1000;

    const layoutStyle = settings.layoutStyle?.mode === 'manual' ? settings.layoutStyle.value : 'auto';
    const headerStyle = settings.headerStyle?.mode === 'manual' ? settings.headerStyle.value : undefined;

    const fonts = settings.fonts?.mode === 'manual' ? settings.fonts.value : undefined;
    const borderRadius = settings.borderRadius?.mode === 'manual' ? settings.borderRadius.value : undefined;

    // Get images & legacy settings
